...
module krome_commons
  ...
  !  KROME 14.08.dev on 2018-06-25 11:46:10
  !  Changeset ded3e8e
  ...
  integer,parameter::nPhotoBins=5
  ...
  real*8::user_crate,user_Av,user_gamma_CO,user_gamma_H2
  !$omp threadprivate(user_crate,user_Av,user_gamma_CO,user_gamma_H2)
  ...
  !xsec for C -> C+ + E
  real*8,allocatable::xsec210_val(:)
  real*8::xsec210_Emin
  real*8::xsec210_idE
  integer::xsec210_n
...
contains
  ...
  function coe(n)
    ...
    !H2+ -> H + H+
    k(226) = small + (1.1d-9*fscale)

    !H3+ -> H2 + H+
    k(227) = small + (4.9d-13*fscale)

    !H3+ -> H2+ + H
    k(228) = small + (4.9d-13*fscale)

    !C- -> C + E
    k(229) = small + (2.4d-7*fscale)

    !CH+ -> C + H+
    k(230) = small + (2.6d-10*fscale)

    !CH2 -> CH + H
    k(231) = small + (7.1d-10*fscale)

    !CH2 -> CH2+ + E
    k(232) = small + (5.9d-10*fscale)

    !CH2+ -> CH+ + H
    k(233) = small + (4.6d-10*fscale)

    !CH3+ -> CH2+ + H
    k(234) = small + (1d-9*fscale)

    !CH3+ -> CH+ + H2
    k(235) = small + (1d-9*fscale)

    !O- -> O + E
    k(236) = small + (2.4d-7*fscale)

    !OH+ -> O + H+
    k(237) = small + (1d-12*fscale)

    !H2O+ -> H2+ + O
    k(238) = small + (5.d-11*fscale)

    !H2O+ -> H+ + OH
    k(239) = small + (5.d-11*fscale)

    !H2O+ -> O+ + H2
    k(240) = small + (5.d-11*fscale)

    !H2O+ -> OH+ + H
    k(241) = small + (1.5d-10*fscale)

    !H3O+ -> H+ + H2O
    k(242) = small + (2.5d-11*fscale)

    !H3O+ -> H2+ + OH
    k(243) = small + (2.5d-11*fscale)

    !H3O+ -> H2O+ + H
    k(244) = small + (7.5d-12*fscale)

    !H3O+ -> OH+ + H2
    k(245) = small + (2.5d-11*fscale)
  ...
  function revHS(Tgas,idx)
    ...
    logical::hasthermoTable(38)
    real*8::yThermoTable(38,200)
    integer::idx
  ...
  subroutine load_arrays()
    ...
    arr_r1(1:275) = (/5,18,18,6,19,19,19,19,6,6,7,7,7,7,34,5,2,5&
        ...
        ,21,26,26,26,27,27,27,29,29,29,14,22,9,9,9,10,10,10,10,30,31&
        ,16,16,16,16,16,16,32,17,17,17,8,8,11,11,11,11,24,24,24,12,12&
        ...
        ,8,8,21,21,21,21,9,9,9,10,5,5,6,8,9,7,2,12,12,14,10,10,16,16&
    ...
    arr_r2(1:275) = (/1,1,1,1,1,1&
        ...
        ,5,5,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38&
        ,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38&
        ,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,38,8,8,9&
        ,9,9,9,22,22,9,5,9/)
    ...
    arr_p1(1:275) = (/18,5,5,19,6,6&
        ...
        ,12,13,14,11,26,27,28,28,4,10,17,16,7,18,19,21,22,20,5,8,26,8&
        ,9,30,10,31,33,9,18,8,5,5,7,20,8,8,12,27,26,27,26,9,9,20,18&
    ...
    arr_p2(1:275) = (/1,38,38,1,38,38,1,18,5,5,5,20,5,5&
        ...
        ,38,38,38,38,38,38,38,38,38,38,38,38,1,1,1,1,1,1,5,1,8,5,1,5&
        ,1,1,9,5,9,5,18,18,5,1,18,5,1,5,5,7,1,18,9,10,7,5,16,10,5,7,1&
        ,1,1,9,1,8,5,2,1,1,5,9,1,5,1,5,5,1,18,38,38,38,38,38,38,38,38&
        ,38,38,38/)
...
call load_xsec("swri_C__C+_E.dat", xsec210_val, xsec210_Emin, xsec210_n, xsec210_idE)
...
do j=1,nPhotoBins
  ...
  !C -> C+ + E
  kk = 0d0
  if(energy_eV>0.0.and.energy_eV<1.d8) kk = xsec_interp(energyL, energyR, xsec210_val(:), xsec210_Emin,xsec210_idE, dshift(idx_C))
  !$omp parallel
  photoBinJTab(3,j) = kk
  !$omp end parallel
...
real*8::lnG0, mu, ntot, Av_1D(1)
real*8::ydata(nPhotoBins)
integer::i
logical, save ::first_call=.true.
real*8,  save ::XH,Jdraine(nPhotoBins),xdata(nPhotoBins), o_f
...
call L1_regression( reshape(xdata(1:ndraine),(/1,ndraine/)), ydata(1:ndraine), &
    Av_1D, lnG0, o_f )
Av = Av_1D(1)
...
integer,parameter::krome_nPhotoBins=5
