...
contains
  ...
  subroutine dump_snapshot(t)
    use parameters, only : ngrid, datadir, pc, spy
    use grid, only : r, n, nHtot, Tgas, Av, dr, dtau
    implicit none
    real*8, intent(in) :: t
    integer :: i, unit
    ...
    do i=1,ngrid
      write(unit,'(200E17.8e3)') t/spy,r(i)/pc,Av(i),Tgas(i),n(:,i),dtau(:,i)
    end do
    write(unit,*)
    close(unit)
